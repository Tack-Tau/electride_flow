...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        --help)
            ...
            echo "  --output-dir DIR           Output directory for phonon jobs (default: ./PHONON_JOBS)"
            echo "  --structure-ids ID1 ID2... Structure IDs to process (REQUIRED, must be RELAX_DONE)"
            echo "  --max-concurrent N         Max concurrent displacement jobs (default: 20)"
...
JOBID=$(sbatch submit_phonon_flow.sh | awk '{print $NF}')
