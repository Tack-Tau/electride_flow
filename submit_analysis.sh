...
BADER_EXE="$HOME/apps/Bader/bader"
OUTPUT_CSV="electride_analysis.csv"
ELF_THRESHOLD=0.6
CONDA_ENV="vaspflow"
...
Options:
  --vasp-jobs DIR        Path to VASP jobs directory (default: VASP_JOBS)
  --bader-exe PATH       Path to bader executable (default: ~/apps/Bader/bader)
  --output FILE          Output CSV file (default: electride_analysis.csv)
  --threshold VALUE      ELF threshold (default: 0.6)
  --conda-env NAME       Conda environment name (default: vaspflow)
...
# Submit job
JOBID=$(sbatch analyze.sh | awk '{print $NF}')
